...
| C2 momentum_breakout | BTC, ETH on HMM=BULL | **RESOLVED 2026-05-18**: Original "0 invocations" inference was a false premise. C2 dispatch IS wired at `trading/live_paper_runner.py:1556-1561` and present in the active runtime image. Zero `[c2]` log lines are explained by (a) all non-event C2 logs are DEBUG-level while container runs INFO, and (b) current market regime (BEAR_TREND, F&G=28) correctly fails C2's `bull_reg + fg_ok` gates every cycle. Strategy is alive and gating honestly. | Gates respected ✓. | No action needed. |
