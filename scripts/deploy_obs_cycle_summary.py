"""
One-shot atomic deploy: OBS CYCLE_SUMMARY observability hook.

Single-file change adding _emit_cycle_summary() helper + per-cycle context
capture in run_crypto(), called at end of main loop.
...
.rollback/2026-05-20_obs_cycle_summary/MANIFEST.txt.

Run: venv\\Scripts\\python scripts\\deploy_obs_cycle_summary.py [--allow-dirty]
"""
...
ROLLBACK_DIR = PROJECT_ROOT / ".rollback" / "2026-05-20_obs_cycle_summary"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="OBS CYCLE_SUMMARY deploy")
    ...
    print("=" * 65)
    print("  OBS CYCLE_SUMMARY deploy -> aaats-paper-crypto")
    ...
    _, summary_check = _run(
        client,
        f"grep -c '_emit_cycle_summary\\|_LAST_CYCLE_CONTEXT' {REMOTE_DIR}/trading/live_paper_runner.py",
        "_emit_cycle_summary / _LAST_CYCLE_CONTEXT refs in deployed file",
    )
    ...
    print(f"  summary refs: {summary_check}")
