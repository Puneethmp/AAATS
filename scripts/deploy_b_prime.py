"""
One-shot atomic deploy: B' apply_kill_switch_gate() + C3/C6 wiring.

Three files shipped together (all-or-nothing):
  trading/live_paper_runner.py
  trading/altcoin_reversion.py
  trading/bollinger_range.py

Doctrine (CLAUDE.md):
  1. SFTP-upload each file to <path>.tmp.
  2. After ALL three .tmp uploads succeed: mv -f for each (atomic on box).
     If any swap fails, roll back the already-swapped ones from .pre baselines.
  3. docker compose up -d --build --no-deps aaats-paper-crypto.

Captures pre/post image SHAs into
.rollback/2026-05-20_b_prime_kill_switch_helper/MANIFEST.txt.

Run: venv\\Scripts\\python scripts\\deploy_b_prime.py [--allow-dirty]
"""
...
FILES = [
    ...
    ("trading/altcoin_reversion.py", "altcoin_reversion.py"),
    ("trading/bollinger_range.py", "bollinger_range.py"),
]

MANIFEST = PROJECT_ROOT / ".rollback" / "2026-05-20_b_prime_kill_switch_helper" / "MANIFEST.txt"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="B' apply_kill_switch_gate deploy")
    parser.add_argument(
        "--allow-dirty",
        action="store_true",
    ...
    print("=" * 65)
    print("  B' apply_kill_switch_gate() deploy -> aaats-paper-crypto")
    print(f"  Target: {USER}@{HOST}:{REMOTE_DIR}/<3 files>")
    print("=" * 65)
    ...
    print("\n[3/7] SFTP upload all 3 files to .tmp...")
    ...
    print("\n[4/7] Atomic swaps (mv -f each)...")
    swapped: list[tuple[str, str, str]] = []
    for rel, remote, tmp in uploaded:
        rc, _ = _run(client, f"mv -f {tmp} {remote} && echo OK", f"swap {rel}")
        if rc != 0:
            print(f"       FAIL on {rel} — rolling back {len(swapped)} already-swapped files")
            # Roll back swapped files from .pre baselines
            rollback_sftp = client.open_sftp()
            try:
                for srel, sremote, _ in swapped:
                    pre_local = PROJECT_ROOT / ".rollback" / "2026-05-20_b_prime_kill_switch_helper" / (_pl.Path(srel).name + ".pre")
                    if pre_local.exists():
                        rb_tmp = sremote + ".rb.tmp"
                        rollback_sftp.put(str(pre_local), rb_tmp)
                        _run(client, f"mv -f {rb_tmp} {sremote} && echo RB_OK", f"rollback {srel}")
                    else:
                        print(f"       WARN: no .pre baseline for {srel} — manual rollback needed")
            finally:
                rollback_sftp.close()
            client.close()
            return 2
    ...
    _, helper_check = _run(
        ...
        "print(\"helper-resolvable\", apply_kill_switch_gate.__name__)' 2>&1",
        "helper resolves inside container",
    )
    ...
    print(f"  helper check: {helper_check}")
