"""
One-shot atomic deploy: trading/live_paper_runner.py -> aaats-paper-crypto.
...
Captures pre/post image SHAs into .rollback/2026-05-15_c5b_halt/MANIFEST.txt.
Verifies the HALTED comment is present in the in-container source post-rebuild.
Run: venv\\Scripts\\python scripts\\deploy_c5b_halt.py [--allow-dirty]
...
LOCAL_FILE = PROJECT_ROOT / "trading" / "live_paper_runner.py"
REMOTE_FILE = f"{REMOTE_DIR}/trading/live_paper_runner.py"
MANIFEST = PROJECT_ROOT / ".rollback" / "2026-05-15_c5b_halt" / "MANIFEST.txt"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="C5b halt single-file deploy")
    parser.add_argument(
        "--allow-dirty",
        action="store_true",
    ...
    print("=" * 65)
    print("  C5b halt deploy -> aaats-paper-crypto")
    ...
    print("\n[7/7] Verifying HALTED comment in container source...")
    rc, halted_line = _run(
        client,
        (
            "docker exec aaats-paper-crypto grep -n "
            "'HALTED 2026-05-15' /app/trading/live_paper_runner.py 2>&1 || "
            "docker exec aaats-paper-crypto grep -n "
            "'HALTED 2026-05-15' trading/live_paper_runner.py 2>&1"
        ),
        "grep HALTED",
    )
    # Also confirm run_funding_arb_crypto is NOT called (commented out only)
    _run(
        client,
        (
            "docker exec aaats-paper-crypto sh -c "
            "\"grep -n 'run_funding_arb_crypto' /app/trading/live_paper_runner.py 2>/dev/null || "
            "grep -n 'run_funding_arb_crypto' trading/live_paper_runner.py 2>/dev/null\""
        ),
        "grep run_funding_arb_crypto (should all be comment lines)",
    )
