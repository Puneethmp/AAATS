...
# C6 has NO validated edge (AUDIT/loss_attribution.md; program closed per
...
def _bollinger_pct_b(
    closes: pd.Series, period: int = BB_PERIOD, n_std: float = BB_STD
...
def run_bollinger_range_crypto(
    ...
    if full_positions is not None and full_portfolio is not None:
        try:
            ...
            log.warning(
                "[c6] kill-switch helper unavailable (%s) — proceeding ungated", exc
            )
            _gate_check = None
            _exit_gate_check = None
    state = _load_state()
    ...
    if ENTRIES_DISABLED:
        log.info(
            "[c6] DEMOTED (research-bed 2026-06-10): entries disabled — "
    ...
    for sym in to_evaluate:
        try:
            ...
            log.info(
                "[c6] %s  price=%.4f  %%B=%.3f  RSI=%.1f  regime=%s",
                sym,
                price,
                pct_b,
                rsi,
            ...
            if pos is not None:
                ...
                # pnl_pct stays the GROSS price move — exit triggers
                # (take_profit/hard_stop) keep their historical semantics.
                # The recorded pnl is NET of round-trip costs (Honest-PnL
                # 2026-06-10, AUDIT/structural_fixes.md FIX 1).
                gross = size_usd * (price - entry_price) / entry_price
                pnl_pct = (price - entry_price) / entry_price
                _exit_notional = size_usd * price / max(entry_price, 1e-9)
                _costs = _round_trip_cost(size_usd, _exit_notional).total
                pnl = gross - _costs
                ...
                if exit_reason:
                    ...
                    try:
                        _record(
                            ...
                            exit_time=exit_ts,
                            pnl_pct=round(pnl / size_usd * 100, 4)
                            if size_usd
                            else None,
                            pct_b=pct_b,
                            rsi=rsi,
            ...
            # ── Check entry ───────────────────────────────────────────────────
                ...
                if open_now >= MAX_CONCURRENT:
                    log.debug(
                        "[c6] %s: max concurrent (%d) reached — skip entry",
                        sym,
                        MAX_CONCURRENT,
                    )
                    continue
                ...
                if open_positions is not None:
                    ...
                    if other and abs(float(other.get("shares", 0.0))) > 1e-9:
                        log.debug(
                            "[c6] %s: another strategy holds position — skip", sym
                        )
                        continue
                ...
                if trade_usd < MIN_TRADE_USD:
                    log.info(
                        "[c6] %s: trade size $%.2f < min $%.2f — skip",
                        sym,
                        trade_usd,
                        MIN_TRADE_USD,
                    )
