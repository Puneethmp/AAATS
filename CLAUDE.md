...
11. **Telegram pre/post alerts silently no-op if token path is wrong** — Phase 2 sqrt-fix deploy (2026-05-27) sourced the bot token from `/srv/aaats/secrets/telegram_bot_token`, which returns HTTP 404 from `api.telegram.org/getMe` (stale/missing). Pre/post-rebuild alerts didn't fire and operator only noticed retrospectively; a real future rebuild that genuinely broke something would go unobserved the same way. Canonical source is `/home/aaats/aaats/.env` vars `ALERTS__TELEGRAM_BOT_TOKEN` + `ALERTS__TELEGRAM_CHAT_ID` (mirrors `scripts/box/aaats-cron-alert.sh`). Use `verify_telegram_path(client)` from `deploy_lib.py` BEFORE any destructive step and `SystemExit` on False; use `send_telegram_message(client, text)` for the actual alert. The `/srv/aaats/secrets/telegram_bot_token` file remains stale; rotate when convenient.
12. **`data/` state files are root-owned — host `echo >` fails Permission denied** — files under the `data/` bind mount (`share_equality_mismatches.json`, `halt_state.json`, etc.) are written by `aaats-paper-crypto` running as `uid=0`, so they land **root-owned on the host**. A host-side `ssh aaats@box 'echo ... > /home/aaats/aaats/data/<file>'` fails with `Permission denied` (the `aaats` SSH user can't write them). Write through the owning container instead — `/app/data` is the same bind mount and the container is root: `ssh aaats@100.95.126.39 'echo "<json>" | docker exec -i aaats-paper-crypto sh -c "cat > /app/data/<file>"'`. Use the `echo … | docker exec -i … cat >` (pipe-to-cat) form, NOT `docker exec … sh -c "echo {…} > …"` — the latter's nested `ssh → sh -c "…"` quoting strips the JSON's double-quotes and writes invalid JSON (empirically verified 2026-05-31). `data/` only, no rebuild, reversible. Source: `docs/known_issues/2026-05-22_share_equality_alert_chain.md` (2026-05-31 entry).
