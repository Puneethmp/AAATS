"""C3c-perp — ETH-anchored C3 on perp economics + funding (B.1.7 Track 5).

An ensemble candidate: the C3c reference-asset variant (z-score of log(ALT/ETH)
instead of log(ALT/BTC)) run on the venue that rescued C3 — Binance perps with
real funding. C3c FAILED hard on spot (PF 0.67); this re-tests whether the
cheaper perp economics rescue the ETH-anchor architecture or whether the
structural failure persists, in the search for a SECOND ensemble candidate
beside C3-perp (the live-flip rule needs >=2).

Differences from run_c3_perp_funded_oos.py:
  - z-score REFERENCE is ETH (eth_df), not BTC. ETH is also the on_bar clock
    (ts_init_delta=1 on ETH bars; alt + BTC bars stay at delta=0, so when the
    ETH clock fires every alt buffer AND the BTC buffer is current).
  - The BTC RSI macro gate is KEPT (BTC freefall still nukes all alts) — it
    reads btc_df, a second reference buffer.
  - Universe unchanged (SOL/LINK/AVAX/DOT). Funding loaded for the universe +
    ETH + BTC; only the traded alts ever carry a position, so funding applies
    to alt longs exactly as in C3-perp.

Same funding machinery as the funded harness (real funding applied to open
longs each 8h settlement, deducted from realized PnL via the on_position_closed
ledger). C3 pure functions reused verbatim; trading/altcoin_reversion.py kept
read-only.

    python3 tools/nautilus/run_c3c_perp_funded_oos.py
...
ETHSYM = "ETH"  # z-score REFERENCE (C3c diff)
BTCSYM = "BTC"  # KEPT for RSI macro gate
...
class C3cPerpFundedStrategy(Strategy):
    ...
    def __init__(self, config, instruments, bar_types, maker: bool):
        ...
        self.funding = {s: load_funding(s) for s in UNIVERSE + [ETHSYM, BTCSYM]}
    ...
    def on_bar(self, bar):
        ...
        if s != ETHSYM:  # C3c diff: ETH is the clock
            return
        self.idx += 1
        if self.maker:
        ...
        eth_df = pd.DataFrame({"close": self.closes[ETHSYM]})  # z-score ref
        btc_df = pd.DataFrame({"close": self.closes[BTCSYM]})  # RSI gate only
...
def run_backtest(maker: bool):
    engine = BacktestEngine(
        config=BacktestEngineConfig(
            trader_id="C3cperp-NT-001", logging=LoggingConfig(bypass_logging=True)
        )
    )
    ...
    for sym in UNIVERSE + [ETHSYM, BTCSYM]:
        ...
        bars = BarDataWrangler(bt, inst).process(
            load_perp_df(sym), ts_init_delta=(1 if sym == ETHSYM else 0)
        )
        engine.add_data(bars)
    strat = C3cPerpFundedStrategy(StrategyConfig(), instruments, bar_types, maker)
