"""C3b-perp — longer-lookback C3 on perp economics + funding (B.1.7 Track 5).

An ensemble candidate: the C3b time-scale variant (120-bar lookback, -1.8
entry, 72h hold) run on the venue that rescued C3 — Binance perps with real
funding. C3b FAILED on spot (PF 0.88) but was OOS-positive; the question is
whether the cheaper perp economics + the period's negative alt funding lift it
over the 1.30 graduation bar, giving the ensemble a SECOND candidate beside
C3-perp (the live-flip rule needs >=2).

Identical machinery to run_c3_perp_funded_oos.py (real perp klines + perp
VIP-0 fees + real funding applied to open longs, gate metrics from the
on_position_closed ledger net of funding). The ONLY differences are the three
C3b overrides below; all other C3 params are read from the c3 module unchanged.

C3 logic is REUSED VERBATIM from trading/altcoin_reversion.py (box module, kept
read-only). The C3b overrides are module-level constants in THIS file, not
mutations of the c3 module.

    python3 tools/nautilus/run_c3b_perp_funded_oos.py
...
# ── C3b overrides (longer time scale) — c3 module untouched ──
C3B_LOOKBACK_BARS = 120  # C3 default is 60
C3B_Z_ENTRY = -1.8  # C3 default is -1.6 (tighter for longer-tail noise)
C3B_TIME_STOP_HOURS = 72  # C3 default is 24
# All other params (Z_HARD_STOP, Z_TARGET, Z_TRAILING_MIN/DROP, MAX_CONCURRENT,
# denylist, cooldown, vol sizing, BTC_RSI_MIN) read from c3 unchanged.
...
class C3bPerpFundedStrategy(Strategy):
    ...
    def on_bar(self, bar):
        ...
        if len(btc_df) < C3B_LOOKBACK_BARS + 5:
            return
        ...
        for sym in UNIVERSE:
            ...
            if len(self.meta) + len(self.pending) >= c3.MAX_CONCURRENT:
                break
            alt_df = pd.DataFrame({"close": self.closes[sym]})
            if len(alt_df) < C3B_LOOKBACK_BARS + 5:
                continue
            z = c3._compute_z_score(alt_df, btc_df, lookback=C3B_LOOKBACK_BARS)
            if z is None or z >= C3B_Z_ENTRY:
                continue
...
def run_backtest(maker: bool):
    engine = BacktestEngine(
        config=BacktestEngineConfig(
            trader_id="C3bperp-NT-001", logging=LoggingConfig(bypass_logging=True)
        )
    )
    ...
    strat = C3bPerpFundedStrategy(StrategyConfig(), instruments, bar_types, maker)
