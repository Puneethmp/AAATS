"""C3-perp WINDOW-ROBUSTNESS test — earlier 6mo window (B.1.7 Track 8).

EXACT CLONE of run_c3_perp_funded_oos.py (the B.1.7 Track 4b graduation harness
that scored PF 1.45 PASS 7/7 on the current window). The ONLY differences are
window coordinates, not strategy logic:
  a. data loaders read the *_earlier.parquet caches (klines AND funding);
  b. OOS_CUTOFF shifts to 2025-03-28 — the 4-month mark of the EARLIER window,
     preserving the identical first-4mo-IS / last-2mo-OOS partition structure
     (this is a window coordinate, NOT a strategy parameter — leaving it at the
     current window's 2026-03-28 would put every earlier-window trade in-sample
     and force a spurious FAIL via an empty OOS split);
  c. emit_report name + engine trader_id renamed for the report artifact.
Strategy class, all params, fee model, FillModel, gate, and metrics extraction
are byte-identical to the current-window harness. This tests whether C3-perp's
graduation is robust across two independent 6mo windows or window-specific.

Original harness header (unchanged logic) ----------------------------------
...
# Track 8: earlier window is 2024-11-28 -> 2025-05-27, so the 4-month mark
# (and IS/OOS boundary) is 2025-03-28 — the window coordinate analogous to the
# current harness's 2026-03-28. Same partition shape, shifted by one year.
...
def load_perp_df(sym: str) -> pd.DataFrame:
    """Real perp 1h klines — EARLIER window cache (*_earlier.parquet)."""
    df = pd.read_parquet(HIST / f"{sym}_USDT_1h_perp_earlier.parquet")
...
def load_funding(sym: str) -> pd.Series:
    f = HIST / f"{sym}_USDT_funding_earlier.parquet"
    if not f.exists():
...
class C3PerpFundedStrategy(Strategy):
    ...
    def __init__(
        ...
        self.maker = maker
        self.gate_version = gate_version  # 0 off | 1 divergence | 2 +correlation
        ...
        self.n_block_div_only = 0  # v2: blocked by divergence signal alone
        self.n_block_corr_only = 0  # v2: blocked by correlation signal alone
        self.n_block_both = 0  # v2: blocked by both signals together
    ...
    def on_bar(self, bar):
        ...
        # gate_version: 0 off | 1 divergence-only (T9) | 2 divergence AND corr (T10)
        if self.gate_version > 0:
            ...
            if divergence is not None:
                ...
                ok = (
                    is_regime_ok(divergence)
                    if self.gate_version == 1
                    else is_regime_ok_v2(divergence, correlation)
                )
                if not ok:
...
def run_backtest(maker: bool, gate_version: int = 0):
    engine = BacktestEngine(
        config=BacktestEngineConfig(
            trader_id="C3perpEarlier-NT-001", logging=LoggingConfig(bypass_logging=True)
        )
    )
...
def evaluate(
    gate_version: int = 0,
    strategy_name: str = "C3_perp_funded_earlier_window",
    emit: bool = True,
    verbose: bool = True,
):
    """Run taker + maker backtests and build the gate metrics dict (earlier window).
    ...
    Default (0) reproduces the ungated Track-8 window-robustness report; the
    ...
    metrics = {
        ...
        "_regime_gate": "v1: 30d divergence>0.08 blocks; v2: AND 30d BTC-alt corr<0.5 blocks",
        "_window": "2024-11-28 to 2025-05-27 (EARLIER 6mo, 1h bars); OOS = last ~60d",
    }
