"""C3b — longer-lookback alt/BTC mean reversion via NT (B.1.7 Track 2).

A C3-class supplement: identical architecture to C3 (BTC-anchored alt/BTC
mean reversion) but on a 2x-longer time scale. C3 uses a 60-bar (2.5d)
lookback; C3b uses 120 bars (5d), with a slightly tighter entry threshold and
a 3x-longer hold limit to match the slower reversion. The hypothesis
(docs/decisions/2026-05-28_b17_c3_supplements_plan.md): BTC-anchored alt/BTC
mean reversion has edge across multiple time scales — a PASS confirms the edge
is robust to lookback and gives the ensemble a time-scale-diversified second
candidate; a FAIL means C3's edge is narrowly tied to its 60-bar config.

Track 1 (C3c, ETH-anchored) already ruled out reference-asset diversification
(it lost money). C3b keeps BTC anchoring — the load-bearing choice — and varies
only the time scale.
...
Minimal diff from tools/nautilus/run_c3_oos.py (B.1.6):
  - C3B_LOOKBACK_BARS = 120 (was c3.LOOKBACK_BARS = 60).
  - C3B_Z_ENTRY = -1.8 (was c3.Z_ENTRY = -1.6) — tighter to compensate for
    longer-tail noise at 2x lookback.
  - C3B_TIME_STOP_HOURS = 72 (was c3.TIME_STOP_HOURS = 24) — longer reversion =
    longer hold.
  - strategy id / trader id / report name renamed to C3b.
  All other params (Z_HARD_STOP, Z_TARGET, Z_TRAILING_MIN/DROP, MAX_CONCURRENT,
  denylist, cooldown, vol sizing, BTC RSI macro gate) are UNCHANGED and read
  straight from the c3 module.

C3b's strategy logic is REUSED VERBATIM from trading/altcoin_reversion.py — the
overrides above are module-level constants in THIS file, not mutations of the c3
module (kept read-only; the box runs it).

    python3 tools/nautilus/run_c3b_oos.py          # full pipeline + gate
...
# ── C3b overrides (longer time scale) — defined HERE, c3 module untouched ──
C3B_LOOKBACK_BARS = 120  # 2x C3's 60 (5 days vs 2.5)
C3B_Z_ENTRY = -1.8  # slightly tighter to compensate for longer-tail noise
C3B_TIME_STOP_HOURS = 72  # 3x C3's 24h hold limit
# All other params (Z_HARD_STOP, Z_TARGET, Z_TRAILING_MIN/DROP, MAX_CONCURRENT,
# denylist, cooldown hours, vol sizing, BTC_RSI_MIN) read from c3 unchanged.
...
class C3bStrategy(Strategy):
    """Drives C3's pure entry/exit logic over NT bar events, longer time scale.

    BTC is the synchronization clock (identical to C3 — C3b is BTC-anchored):
    alt bars for timestamp T are processed before the BTC bar for T (BTC bars
    get ts_init_delta=1), so when the BTC bar fires every alt buffer is current
    as of T — no look-ahead. The only difference from C3 is the longer lookback
    / tighter entry / longer hold (the C3B_* constants).
    ...
    def on_bar(self, bar):
        ...
        if len(btc_df) < C3B_LOOKBACK_BARS + 5:  # C3b diff: 120+5 buffer
            return

        # ---- EXIT phase (always MARKET so positions close) ----
        for sym in list(self.meta):
            alt_df = pd.DataFrame({"close": self.closes[sym]})
            z = c3._compute_z_score(alt_df, btc_df, lookback=C3B_LOOKBACK_BARS)
            if z is None:
                continue
            m = self.meta[sym]
            if z > m["max_z"]:
            ...
            if z >= c3.Z_TARGET:
                ...
                reason = "z_hard_stop"
            elif age >= C3B_TIME_STOP_HOURS:  # C3b diff: 72h hold limit
                reason = f"time_stop_{C3B_TIME_STOP_HOURS}h"
            if reason:
                ...
                if (
                    reason in ("z_hard_stop", f"time_stop_{C3B_TIME_STOP_HOURS}h")
        ...
        for sym in UNIVERSE:
            ...
            if len(self.meta) + len(self.pending) >= c3.MAX_CONCURRENT:
                break
            alt_df = pd.DataFrame({"close": self.closes[sym]})
            if len(alt_df) < C3B_LOOKBACK_BARS + 5:  # C3b diff: 120+5 buffer
                continue
            z = c3._compute_z_score(alt_df, btc_df, lookback=C3B_LOOKBACK_BARS)
            if z is None or z >= C3B_Z_ENTRY:  # C3b diff: -1.8 entry threshold
                continue
...
def _metrics_from_positions(positions_df, start_capital=START_CAPITAL):
    ...
    for _, p in positions_df.iterrows():
        ts = p["ts_closed"]
        # C3b diff: with the 72h hold, positions opened in the final stretch can
        # still be OPEN at backtest end -> ts_closed is NaT. Those are not closed
        # round-trips, so they're excluded from the gate metrics (G4 counts
        # CLOSED trades). C3's 24h hold closed everything in-window, so the
        # original harness never hit this.
...
def run_backtest(maker: bool):
    engine = BacktestEngine(
        config=BacktestEngineConfig(
            trader_id="C3b-NT-001", logging=LoggingConfig(bypass_logging=True)
        )
    )
    ...
    engine.add_strategy(C3bStrategy(StrategyConfig(), instruments, bar_types, maker))
    engine.run()
...
def main():
    ...
    metrics = {
        ...
        "_config": (
            f"lookback={C3B_LOOKBACK_BARS} z_entry={C3B_Z_ENTRY} "
            f"time_stop={C3B_TIME_STOP_HOURS}h (C3b longer-lookback)"
        ),
